...
import transaction
import pytest
from octojobs.models import Job, get_tm_session
...
def add_models(dummy_request):
    ...
    """
    dummy_request.dbsession.add_all(JOBS)
...
def testapp():
    ...
    app = main({}, **{"sqlalchemy.url": 'postgres://colinlamont@localhost:5432/octojobs'})
    testapp = TestApp(app)
...
def test_new_jobs_are_added(db_session):
    """New jobs get added to the database."""
    db_session.add_all(JOBS)
    query = db_session.query(Job).all()
    assert len(query) == len(JOBS)
